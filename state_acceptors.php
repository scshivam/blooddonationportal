                        ...
                        <div class="panel-heading">
							...
							$st=$_POST['state'];?>
                              <table class="table-striped table-hover col-lg-12" id="dataTables-example">
								...
								$row1=mysql_fetch_assoc($res1);
								$st=$row1['State_Name'];
								$query="Select Name,Age,Sex,BGroup,Address,City,State,pincode,phone,Alternate,email,Detail from donor where Type='AC' AND State='$id' And Last_Donated!='Y'";
...
<button class="btn btn-primary" value="<?php echo $id; ?>">Unable To Reach</button></form></center>										  </div>
								...
								<?php }
									...
									{	
										$query="Select Id,Name,Age,Sex,BGroup,Address,City,State,pincode,phone,Alternate,email,Detail from donor where Type='AC' AND State='$st' AND Last_Donated!='Y'";
							$res=mysql_query($query)or die(mysql_error());
							while($row=mysql_fetch_assoc($res))
							{
								$email=$row['email'];
